...
final class CognitoIdentityDemoTests: XCTestCase {
    ...
    func testFindNonexistent() {
        identityTester?.getIdentityPoolID(name: "BogusPoolIsBogus", createIfMissing: false) { poolID in
    ...
    func testCreateThenFind() {
        ...
        identityTester?.getIdentityPoolID(name: "testCreateThenFind", createIfMissing: true) { poolID in
            firstPoolID = poolID
            XCTAssertNotNil(firstPoolID, "Unable to create or obtain test pool")
        }
        
        identityTester?.getIdentityPoolID(name: "testCreateThenFind", createIfMissing: false) { poolID in
            secondPoolID = poolID
